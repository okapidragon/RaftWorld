...
def swimTrouble(player, taskName):
    player.health -= 12
    imp.outputMessage.append(f"You spent too long in the water during {taskName} and took damage.", color = "Red")
    player.setHealth()
...
class Task:
    ...
    async def execute(self, player):
        ...
        for neededItem in self.neededItems:
            ...
            if breaked:
                if neededItem.name == "Fishing Rod" and not (inv.lookup("Seaweed") in imp.displayedResources):
                    initializeSeaweed()
